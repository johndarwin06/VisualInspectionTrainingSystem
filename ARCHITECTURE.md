...
Database integration is an opt-in boundary. It requires an explicitly named test schema and connection string supplied outside source control, rejects the known operational schema, disables pooling, and skips closed when the safety contract is not satisfied. Planned database-testing Issue #23 owns provisioning that isolated schema. Until then, the four database tests in each Debug and Release run remain skipped and must not be executed against production data.
